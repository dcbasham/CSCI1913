...
def sorted_put(pricebook, name, price):
    ...
    a float digit price and string name. Update pricebook to reflect the new price
    and keep sorted order"""
    new_tuple = (price, name)
    last = len(pricebook) - 1
    low = len(pricebook) - 1
    high = 0
    count = 0
    if len(pricebook) >= 1:
        while high <= low:
            mid = (low + high) // 2
            if pricebook[mid][1] == name:
                pricebook.insert(count, new_tuple)
                pricebook.pop(count + 1)
                print('done')
                return pricebook
            elif pricebook[mid][1] < name:
                low = mid - 1
